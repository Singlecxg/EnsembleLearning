...
def printPredictResult(predict, y_test, options):
	...
	for i in range(len(y_test)):
		RMSE += math.pow( (y_test[i] - predict[i]), 2)
	RMSE = RMSE / len(y_test)
	math.sqrt(RMSE)
	print 'ensemble: %s, classifier: %s, features: %s, normalize: %d, T: %d. RMSE: %.8f'%(options.ensemble, options.classifier, options.features, options.normalize, options.T, RMSE)
...
if __name__ == '__main__':
	...
	# 读取数据，划分数据集
	data = np.loadtxt('Test_train_vector.txt')
	X, y = data[:, :-1], data[:, -1:].reshape(-1, ).astype("int32")
